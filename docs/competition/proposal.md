...
| **GitHub 仓库** | https://github.com/oilleelssq-wq/moonmarkdown |
...
- 可配置选项 API（GFM / HTML / 实体解码 / 引用链接 / 语法高亮 / CSS 预设）；
- CSS 预设（None / Default / Bootstrap），渲染器注入 class 属性；
- 完整 HTML 渲染器（含 HTML 实体转义、原始 HTML 开关）；
- 代码语法高亮（支持 MoonBit / JavaScript / Python / Rust / Go / Java / C / HTML / CSS）；
- CLI 命令行工具（支持文件输入/输出、--gfm、--highlight、--no-html、--css）；
- 集成 CommonMark 官方 651 个 Spec 测试用例，当前通过 241 个（37%），基线回归保护；
...
| 规范兼容 | 207/542 (38%) | 542/542 (通过 C) | **目标 400+/651 (61%+)** |
| 平台支持 | Wasm/JS | 受限 | **Native / Wasm / JS** |
| 扩展 | GFM 部分 | 无 | **GFM 完整 + 可配置选项** |
...
1. **已完成**：AST 类型定义、块级解析器（标题/段落/代码块/引用/列表）、行内解析器（转义/代码片段/强调/链接/自动链接）、HTML 渲染器、GFM 表格/任务列表/删除线、可配置选项 API、CSS 预设、语法高亮、CLI 重写（选项处理）、651 条 Spec 测试集成（238 通过，基线回归保护）
2. **进行中**：Spec 合规率持续提升
3. **计划**：stdin 支持（native target）、合规率冲刺
